...
public class TriosHandPanel extends JPanel {
  ...
  public TriosHandPanel(int rows, int cols, ReadOnlyTriosModel model, Player owner) {
    setLayout(new BoxLayout(this, BoxLayout.Y_AXIS)); // Set the layout to Y_AXIS for vertical arrangement
    this.model = model;
    this.setBorder(BorderFactory.createLineBorder(Color.BLACK)); // Border around the entire panel

    ArrayList<CardCell> cardCells = new ArrayList<CardCell>();
    for (int i = 0; i < model.getHand(owner).size(); i++) {
      cardCells.add(new CardCell(500 / rows, 500 / cols, (ThreeTriosCard) this.model.getHand(owner).get(i)));
    }
